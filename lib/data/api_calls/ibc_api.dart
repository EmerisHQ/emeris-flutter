...
class IbcApi {
  ...
  Future<Either<GeneralFailure, PrimaryChannelJson>> getPrimaryChannel({
    ...
    try {
      final response = await _dio.get('https://dev.demeris.io/v1/chain/$chainId/primary_channel/$destinationChainId');
  ...
  Future<Either<GeneralFailure, ChainJson>> getChainDetails(String chainId) async {
    try {
      final response = await _dio.get('https://dev.demeris.io/v1/chain/$chainId');
