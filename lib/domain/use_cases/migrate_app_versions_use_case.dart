...
import 'package:flutter_app/data/local/codecs/app_info_codec.dart';
...
class MigrateAppVersionsUseCase {
  ...
  Future<Either<MigrateAppVersionsFailure, Unit>> execute() async {
    final currentAppInfo = await _appInfoProvider.appInfo;
    final appInfoResult = await _getSavedAppInfo();
    if (appInfoResult.isLeft()) {
      return appInfoResult.map((_) => unit);
    }
    final appInfo = appInfoResult.getOrElse(() => currentAppInfo.copyWith(buildNumber: '0')) ?? AppInfo.empty();
    ...
    return Stream.fromIterable(migrations)
        ...
        .flatMap(
          (_) async => _saveAppInfo(currentAppInfo, newBuildNumber: currentAppInfo.buildNumberInt).mapError(
            (fail) => const MigrateAppVersionsFailure.unknown(),
          ),
        );
  }
